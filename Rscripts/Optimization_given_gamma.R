...
loglike = function(alpha, beta, gamma = 1, X, y, t, delta){ # Loglikelihood
...
loglike_neg = function(parameter, gamma = 1, X, y, t, delta){
...
tmp = lapply(df, function(mydf){
  ...
  res = optim(
    par = c(1, 1, 1, 1, 1), # Initialization
  ...
  z = cbind(c('a1', 'a2', 'intercept', 'z1', 'z2'), res$par) %>% as.data.frame()
